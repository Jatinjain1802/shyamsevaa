...
const Panchang = () => {
    ...
    useEffect(() => {
        const fetchPanchang = () => {
            try {
                ...
                // SunCalc handles Moon events which mhah-panchang currently doesn't provide
                const moonTimes = SunCalc.getMoonTimes(date, lat, lon);

                // Formatting Helpers
                const formatTime = (timeStr) => {
                    if (!timeStr || isNaN(new Date(timeStr).getTime())) return "--:--";
                ...
                // Calculate Muhurta length for Abhijit (1/15th of day)
                ...
                // Calculate Octant length for Rahu/Guli/Yama (1/8th of day)
                const octantLengthMs = dayLengthMs / 8;
                const dayIndex = date.getDay(); // 0 is Sunday

                // Vedic Rules for 8-fold divisions (Day-time)
                const rahuMap = [8, 2, 7, 5, 6, 4, 3];
                const yamaMap = [5, 4, 3, 2, 1, 7, 6];
                const guliMap = [7, 6, 5, 4, 3, 2, 1];

                const getPeriod = (partNum) => ({
                    start: formatTime(new Date(sunriseDate.getTime() + (partNum - 1) * octantLengthMs)),
                    end: formatTime(new Date(sunriseDate.getTime() + partNum * octantLengthMs))
                });
                ...
                const data = {
                    ...
                    rahu: getPeriod(rahuMap[dayIndex]),
                    gulika: getPeriod(guliMap[dayIndex]),
                    yamaganda: getPeriod(yamaMap[dayIndex]),
                    abhijit: {
    ...
    // LEARNING: SEO Dynamic Metadata
    // We use useEffect to update the document title and meta tags when the component mounts or translations change.
    useEffect(() => {
        if (!panchangData) return;

        // 1. Set Document Title
        document.title = `${t('panchang.title')} - ${currentDate} | Shyampuja`;

        // 2. Manage Meta Description
        let metaDescription = document.querySelector('meta[name="description"]');
        const descriptionText = `Get today's accurate Vedic Panchang for ${currentDate}. Current Tithi: ${panchangData.paksha} ${panchangData.tithi}, Nakshatra: ${panchangData.nakshatra}. Check Shubh Muhurat, Rahu Kaalam, and celestial timings.`;
        ...
        script.id = 'panchang-schema';
        script.innerHTML = JSON.stringify(schema);
        document.head.appendChild(script);

        return () => {
            // Cleanup on unmount
            const existingScript = document.getElementById('panchang-schema');
            if (existingScript) existingScript.remove();
        };
    }, [t, panchangData, currentDate]);
            ...
            <div className="max-w-[1400px] mx-auto px-6 md:px-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    ...
                    <div className="lg:col-span-8 h-full">
                        <div className="glass-card rounded-[3rem] p-10 border border-marigold/10 shadow-lg relative overflow-hidden bg-white/40 h-full">
                    ...
                    <div className="lg:col-span-4 h-full">
                        {/* Shubh Muhurat Card */}
                        <div className="bg-linear-to-br from-green-600 to-green-700 rounded-[2.5rem] p-8 text-white shadow-xl relative overflow-hidden group h-full flex flex-col justify-center">
                ...
                {/* Caution Periods - Full Width Long Card */}
                <div className="mt-10">
                    <div className="glass-card rounded-[3rem] p-12 border border-red-100 bg-white/60 shadow-lg relative overflow-hidden">
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-6 mb-12">
                            ...
                            <div>
                                <h3 className="text-3xl text-heritage-dark font-serif">{t('panchang.caution')}</h3>
                                <p className="text-stone-500 text-sm">Periods traditionally considered less auspicious for new beginnings</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            <div className="flex justify-between items-center p-6 rounded-[2rem] bg-red-50/50 border border-red-100 group hover:bg-red-50 transition-all hover:scale-[1.02]">
                                <div className="flex flex-col">
                                    <span className="text-xs text-red-400 font-bold uppercase tracking-wider mb-2">{t('panchang.rahu_kaalam')}</span>
                                    <span className="text-2xl text-heritage-dark font-black font-serif">{panchangData.rahu.start} - {panchangData.rahu.end}</span>
                                </div>
                                <Info className="w-6 h-6 text-red-300" />
                            </div>

                            <div className="flex justify-between items-center p-6 rounded-[2rem] bg-orange-50/50 border border-orange-100 group hover:bg-orange-50 transition-all hover:scale-[1.02]">
                                <div className="flex flex-col">
                                    <span className="text-xs text-orange-400 font-bold uppercase tracking-wider mb-2">{t('panchang.yamaganda')}</span>
                                    <span className="text-2xl text-heritage-dark font-black font-serif">{panchangData.yamaganda.start} - {panchangData.yamaganda.end}</span>
                                </div>
                                <Info className="w-6 h-6 text-orange-300" />
                            </div>

                            <div className="flex justify-between items-center p-6 rounded-[2rem] bg-yellow-50/50 border border-yellow-100 group hover:bg-yellow-50 transition-all hover:scale-[1.02]">
                                <div className="flex flex-col">
                                    <span className="text-xs text-yellow-500 font-bold uppercase tracking-wider mb-2">{t('panchang.gulika_kaalam')}</span>
                                    <span className="text-2xl text-heritage-dark font-black font-serif">{panchangData.gulika.start} - {panchangData.gulika.end}</span>
                                </div>
                ...
                {/* Educational Section */}
                <div className="mt-20">
                    <div className="bg-heritage-dark rounded-[3rem] p-12 text-white relative overflow-hidden">
                        ...
                        <p className="text-white/70 leading-loose text-lg mb-10 max-w-4xl">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                            {[
                                ...
                                <div key={i} className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/10">
                                    <CheckCircle2 className="w-6 h-6 text-marigold shrink-0" />
